// ==UserScript==
// @name			CancelBattle_HeroWars_EN
// @name:en			CancelBattle_HeroWars_EN
// @namespace		CancelBattle_HeroWars_EN
// @version			2.033
...
(function() {
	...
	let checkboxes = {
		passBattle: {
			label: 'Skip battle',
			cbox: null,
			title: 'Skip battle in Titan Walley and Outland, autoskip battle in Campaign and Tower',
			default: false
		},
		fixBanError: {
			label: 'fixBanError',
			cbox: null,
			title: 'Remove BAN ERROR, when you banned for cheating in Asgard or GuildWar',
			default: false
		},
		endlessCards: {
			label: 'Unlim Devination Card',
			cbox: null,
			title: 'Unlimited Devination Card in Dungeon',
			default: false
		},
		sendExpedition: {
			label: 'AutoExpeditions',
			cbox: null,
			title: 'Auto send Expeditions, when log in game',
			default: false
		},
		cancelBattle: {
			label: 'Cancel Battle',
			cbox: null,
			title: 'Cancellation any battle in Asgard, GW/CW (you may be banned!!!)',
			default: false
		},
		getAutoGifts: {
			label: 'Gifts',
			cbox: null,
			title: 'Auto get all day gifts, when log in game',
			default: true
		},
		preCalcBattle: {
			label: 'Battle postcalc',
			cbox: null,
			title: 'Recalc battle and show win/loss chance for skip battle',
			default: false
		},
		countControl: {
			label: 'Box open amount',
			cbox: null,
			title: 'Ability to specify the number of open "Loot Boxes"',
			default: false
		},
		repeatMission: {
			label: 'Repeat in Campaign',
			cbox: null,
			title: 'AutoRepeat battle until the power runs out',
			default: false
		},
	};
	/** Инпуты */
			...
			for (const call of testData.calls) {
				...
				if ((call.name == 'adventure_endBattle' ||
					...
					if (!call.args.result.win) {
						...
						if (call.name == 'crossClanWar_endBattle' ||
							...
							call.name == 'adventureSolo_endBattle') {
							resultPopup = await showMsgs('You defeated!', 'Ok', 'Cancel', 'Auto');
						} else {
							resultPopup = await showMsg('You defeated!', 'Ok', 'Cancel');
						}
						if (resultPopup) {
				...
				if (call.name == 'clanRaid_endBossBattle' &&
					...
					let resultPopup = await showMsgs(
						'You have ' + sumDamage.toLocaleString() + ' damage.',
						'Ok', 'Cancel', 'Auto')
					if (resultPopup) {
						fixBattle(call.args.progress[0].attackers.heroes);
						fixBattle(call.args.progress[0].defenders.heroes);
				...
				if (isChecked('repeatMission') &&
					...
					setTimeout(async () => {
						if (!isSendsMission && await popup.confirm('Repeat mission?', [
								{msg: 'Yes', result: true},
								{msg: 'No', result: false},
							])) {
				...
				if (isChecked('countControl') &&
					...
					call.args.amount > 1) {
					const result = await popup.confirm('Open amount:', [
							{msg: 'Open', isInput: true, default: call.args.amount},
						]);
					if (result) {
		...
		try {
			...
			for (const call of respond.results) {
				...
				if ((call.ident == callsIdent['clanWarAttack'] ||
					...
					Promise.all(actions)
						.then(e => {
							...
							setProgress((firstBattle ? 'Win' : 'Loss') + ' ' + countWin + '/' + e.length + ' X', false, hideProgress)
						});
				}
				...
				if (isChecked('countControl') && call.ident == callsIdent['consumableUseLootBox']) {
					...
					if (newCount && await popup.confirm('Open ' + newCount + ' box recurse?', [
							{msg: 'Yes', result: true},
							{msg: 'No', result: false},
						])) {
	...
	function createInterface() {
		...
		const details = scriptMenu.addDetails('Settings')
	...
	const buttons = {
		...
		sendExpedition: {
			name: 'Expeditions',
			title: 'Send expeditions and autoget rewards',
			func: checkExpedition,
		},
		newDay: {
			name: 'Sync data',
			title: 'Partial synchronicity of the game data without reloading game (use after get reward, Dungeon, Titan Valley)',
		...
		offerFarmAllReward: {
			name: 'Special gift',
			title: 'Get all special gifts and "Easter eggs"',
		...
		testRaidNodes: {
			name: 'Asgard raid',
			title: 'AutoBattle for day raid (you need set all packs)',
			func: function () {
				confShow('Start Asgard raid?', testRaidNodes);
			},
		},
		goToSanctuary: {
			name: 'Go Sanctuary',
			title: 'Fast open Sanctuary',
			func: cheats.goSanctuary,
		},
		goToClanWar: {
			name: 'Go Guild War',
			title: 'Fast open Guild War',
	...
	function executeTower(resolve, reject) {
		...
		function checkFloor(towerInfo) {
			...
			setProgress('Tower: floor ' + floorNumber);
			if (floorNumber > 49) {
				if (isOpenChest) {
	...
	function executeTitanArena(resolve, reject) {
		...
		function checkTier(titanArena) {
			...
			if (currTier) {
				setProgress('Titan Valley: Level ' + currTier);
			}
		...
		function resultTitanArenaEndBattle(e) {
			...
			let numReval = countRivalsTier - finishListBattle.length;
			setProgress('Titan Valley: Leval ' + currTier + '</br>Battle: ' + numReval + '/' + countRivalsTier + ' - ' + attackScore);
		...
		fetch(url).then(
			...
			data => {
				...
				send(JSON.stringify(freebieCheckCalls), e => {
					...
					console.log('Gifts: ' + countGetGifts);
				});
			}
		)
	}
	...
	function bossRatingEvent() {
		...
		send(heroGetAllCall, function (data) {
			...
			send('{"calls":[' + calls + ']}', e => {
				console.log(e);
				setProgress('Used ' + e?.results?.length + ' heroes', true);
	...
	function rewardBossRatingEvent() {
		...
		send(rewardBossRatingCall, function (data) {
			...
			setProgress('Enemy killed: ' + score);
			...
			if (!count) {
				setProgress('Nothing get', true);
				return;
			}

			send(JSON.stringify(getRewardCall), e => {
				console.log(e);
				setProgress('Get ' + e?.results?.length + ' rewards', true);
			});
		});
	}
	...
	function offerFarmAllReward() {
		...
		send(offerGetAllCall, function (data) {
			...
			send(JSON.stringify(rewardListCall), e => {
				console.log(e);
				setProgress('Get ' + e?.results?.length + ' reward', true);
			});
		});
	}
	/** Собрать запределье */
		...
		return new Promise(function (resolve, reject) {
			send('{"calls":[{"name":"bossGetAll","args":{},"ident":"bossGetAll"}]}', e => {
				...
				if (!bossRaidOpenChestCall.calls.length) {
					setProgress('Outland passed', true);
					resolve();
					return;
				}
		...
		return new Promise(function (resolve, reject) {
			...
			send(JSON.stringify(questGetAllCall), function (data) {
				...
				if (!questAllFarmCall.calls.length) {
					setProgress('Get reward: ' + number, true);
					resolve();
					return;
				}
	...
	function executeRaidNodes(resolve, reject) {
		...
		function checkNodes() {
			setProgress('Attempts left: ' + raidData.attempts);
		...
		function endRaidNodes(reason, info) {
			isCancalBattle = true;
			let textCancel = raidData.cancelBattle ? ' Battle canceled: ' + raidData.cancelBattle : '';
			setProgress('Raid end!' + textCancel, true);
			console.log(reason, info);
			resolve();
		}
	}
	...
	this.sendsMission = async function (param) {
		if (isStopSendMission) {
			...
			console.log('Stopped');
			setProgress('');
			await popup.confirm('Stopped<br>Rerun: ' + param.count, [{
				msg: 'Ok',
				result: true
			}, ])
			return;
		...
		SendRequest(JSON.stringify(missionStartCall), async e => {
			if (e['error']) {
				...
				console.log(e['error']);
				setProgress('');
				let msg = e['error'].name + ' ' + e['error'].description + '<br>Rerun: ' + param.count;
			...
			BattleCalc(e.results[0].result.response, 'get_tower', async r => {
				...
				SendRequest(JSON.stringify(missionEndCall), async (e) => {
					...
					if (r['error']) {
						...
						console.log(r['error']);
						setProgress('');
						await popup.confirm('Rerun: ' + param.count + ' 3 ' + r['error'], [
							{msg: 'Ok', result: true},
						])
						return;
					}
					...
					param.count++;
					setProgress('Mission passed: ' + param.count + ' (stop)', false, () => {
	...
	function executeBossBattle(resolve, reject) {
		...
		async function resultPreCalcBattle(damages) {
			...
			const result = await popup.confirm(
				'Damage stats:' +
				'<br>Minimum: ' + minDamage.toLocaleString() +
				'<br>Maximum: ' + maxDamage.toLocaleString() +
				'<br>Average: ' + avgDamage.toLocaleString() +
				'<br>Finding damage more than ' + reachDamage.toLocaleString(), [
				{msg: 'No', result: 0},
				{msg: 'Go', isInput: true, default: reachDamage},
			])
			if (result) {
				reachDamage = result;
		...
		async function resultBattle(e) {
			...
			if (resultDamage > reachDamage && await popup.confirm(countBattle + ') Damage ' + resultDamage.toLocaleString(), [
				{msg: 'Ок', result: true},
				{msg: 'Get more', result: false},
			]))  {
				endBattle(e, false);
				return;
			}
		...
		function endBattle(battleResult, isCancal) {
			...
			send(JSON.stringify({calls}), e => {
				...
				scriptMenu.setStatus('Boss killed! Damage: ' + lastDamage);
				setTimeout(() => {
					scriptMenu.setStatus('');
				}, 5000);
				endBossBattle('Ok');
	...
	function executeAutoBattle(resolve, reject) {
		...
		function resultPreCalcBattle(results) {
			...
			setProgress('Not fate, odds are 0', true);
			endAutoBattle('Not this time');
		}
		/** Начало боя */
		function startBattle() {
			countBattle++;
			...
			if (countBattle > countMaxBattle) {
				setProgress('Exceeded attempt limit: ' + countMaxBattle, true);
				endAutoBattle('Exceeded attempt limit: ' + countMaxBattle)
				return;
			}
			let calls = [{
		...
		function endBattle(battleResult, isCancal) {
			...
			send(JSON.stringify({
				...
				scriptMenu.setStatus('Win!');
				setTimeout(() => {
					scriptMenu.setStatus('');
				}, 5000)
				endAutoBattle('Success!')
			});
		}
		/** Завершение задачи */
